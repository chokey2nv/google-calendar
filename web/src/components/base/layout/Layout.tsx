...
import type { EventListProps } from "@/components/EventList";
...
export const AppLayout:FC<AppLayoutProps> = ({ children }) => {
    ...
    const onDelete: EventListProps["onDelete"] = async (eventId) => {
    ...
    const onEdit: EventListProps["onEdit"] = (event) => {
        onOpenModal({
            calendarEvent: event,
            isEdit: true,
            onSubmit: async (calendarEvent) => {
                await eventHook.updateEvent({eventId: event.id, event: calendarEvent})
                const events = await eventHook.getEvents() 
                if(events){
                    setEvents(events)
                }
    ...
    return <Root>
        ...
        <Content>
            ...
            {!isMobile && isMenuOpen && (
                ...
                >
                    <Sidebar {...{ onCreateEvent, onDelete, onEdit }}/>
                </MotionSidebar>
            )}
            {isMobile && (
                <Drawer
